...
const JoinBusiness = ({ navigate }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    // FormData로 모든 값 추가 (파일 포함)
    const formData = new FormData();
    formData.append('corpName', corpName);
    formData.append('corpRegNum', corpRegNum);
    formData.append('ceo', ceo);
    formData.append('dept', dept);
    formData.append('manager', manager);
    formData.append('phone', phone);
    formData.append('email', email);
    formData.append('corpTel', corpTel);
    formData.append('address', address);
    formData.append('id', id);
    formData.append('pw', pw);
    if (bizCert) formData.append('bizCert', bizCert);

    try {
      const res = await fetch('http://localhost:3001/user/joinCorp', {
        method: 'POST',
        body: formData, // Content-Type 자동 세팅(multipart/form-data)
      });
      if (res.ok) {
        alert('회원가입 성공!');
